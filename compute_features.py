...
def compute_features(
    ...
    """
    Compute social and map features for the sequences.

    Args:
        seq_path (str): file path for the sequence whose features are to be computed.
        map_features_utils_instance: MapFeatureUtils instance.
    
    Returns:
        merged_features (numpy array): SEQ_LEN X NUM_FEATURES
        map_features_helpers (dict): Dictionary containing helpers for map features
    """
    ...
    if agent_track.shape[0] == args.obs_len:
        ...
        merged_features = np.concatenate(
            (agent_track_seq, map_features), axis=1)
    else:
        merged_features = np.concatenate(
            (agent_track, map_features), axis=1)
...
def load_seq_save_features(
    ...
    for seq in sequences[start_idx: start_idx + args.batch_size]:
        ...
        print(
            f"{args.mode}:{count}/{args.batch_size} with start {start_idx} and end {start_idx + args.batch_size}: {seq}"
        )
