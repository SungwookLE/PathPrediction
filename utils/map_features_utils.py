...
class MapFeaturesUtils:
    ...
    def get_point_in_polygon_score(self, lane_seq: List[int],
        ...
        """
        Get the number of coordinates that lie inside the lane seq polygon
    ...
    def sort_lanes_based_on_point_in_polygon_score(
        ...
        sorted_point_in_polygon_scores_idx = np.lexsort(
            (randomized_tiebreaker, np.array(point_in_polygon_scores)))[::-1] # 거꾸로 뒤집기
        sorted_lane_seqs = [
        ...
        if viz:
            lane_seq_polygon = cascaded_union([
                Polygon(avm.get_lane_segment_polygon(lane, city_name)).buffer(0)
                for lane in sorted_lane_seqs[0]
            ])

            # plot each polygon shape directly
            plt.plot(*lane_seq_polygon.exterior.xy)
            title = [str(txt)+" " for txt in lane_seq]
            plt.title(title)
            plt.show()
    ...
    def get_candidate_centerlines_for_trajectory(
        ...
        Returns:
            candidates_centerlines: List of candidate centerlines
        """ 
        ...
        while (len(curr_lane_candidates) <1 and self._MANHATTAN_THRESHOLD < max_search_radius):
            ...
            curr_lane_candidates = avm.get_lane_idx_in_xy_bbox(
                xy[-1,0], xy[-1,1], city_name, self._MANHATTAN_THRESHOLD)
        ...
        # Assuming a speed of 50mps, set threshold for traversing in the front and back
        ...
        obs_pred_lanes, scores = self.sort_lanes_based_on_point_in_polygon_score(obs_pred_lanes, xy, city_name, avm, viz=False)
    
        # If the best centerline is not along the direction of travel, re-sort
        if mode == "test":
            candidates_centerlines = self.get_heuristic_centerlines_for_test_set(
                obs_pred_lanes, xy, city_name, avm, max_candidates, scores)
        else:
            candidates_centerlines = avm.get_cl_from_lane_seq(
        ...
        if viz:
            ...
            for centerline_coords in candidates_centerlines:
            ...
            plt.ylabel("Map Y")
            plt.axis('off')
            plt.title(f"Number of candidates = {len(candidates_centerlines)}")
            plt.show()
        
        return candidates_centerlines
    ...
    def compute_map_features(self,
                             ...
                             mode: str) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        Returns:
            ...
            map_feature_helpers (dict): Dictionary containing for map features
        
        """
        # Get observed 2secs of the agent
        agent_xy = agent_track[:, [raw_data_format["X"], raw_data_format["Y"]]].astype("float")
        agent_track_obs = agent_track[:obs_len]
        agent_xy_obs = agent_track_obs[:, [raw_data_format["X"], raw_data_format["Y"]]].astype("float")
